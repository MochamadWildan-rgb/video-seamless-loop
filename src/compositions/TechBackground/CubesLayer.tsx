...
const CUBE_COUNT = 100; // Meningkatkan jumlah kubus untuk kompleksitas
...
const Cube: React.FC<{
    ...
    const faceStyle: React.CSSProperties = {
        ...
        backgroundColor: 'rgba(0, 40, 70, 0.15)',
...
export const CubesLayer: React.FC = () => {
    ...
    const cubes = React.useMemo((): CubeInfo[] => {
        return Array.from({ length: CUBE_COUNT }).map((_, i) => ({
            ...
            zOffset: random(`cube-z-${i}`) * 1000 - 500,
        }));
    }, [width, height]);
    ...
    return (
        ...
        >
            {cubes.map((cube, i) => {
                const progress = frame / 600; // 10 detik * 60 fps
                ...
                // Gerakan maju-mundur pada sumbu Z untuk efek kedalaman
                const zPosition = Math.sin(progress * 2 * Math.PI) * 200 + cube.zOffset;
